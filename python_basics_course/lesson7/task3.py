...
соответствующий количеству ячеек клетки (целое число). В классе должны быть реализованы методы перегрузки арифметических операторов: сложение (__add__()), вычитание (__sub__()),
...
class Cell:
    ...
    def make_order(self, column):
        return '\n'.join(['*' * column for _ in range(self.number_of_cells // column)]) + '\n' + '*' * (
                    self.number_of_cells % column)

    def __sub__(self, other):
        if self.number_of_cells - other.number_of_cells > 0:
            self.number_of_cells = self.number_of_cells - other.number_of_cells
            return Cell(self.number_of_cells)
        else:
            print('Fatal ERROR, cells not found')
